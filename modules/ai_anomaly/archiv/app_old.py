...
def get_model():
    ...
    if _model is None:
        ...
        # Trainiere auf normalem Spektrum (Gauß'sches Rauschen)
        np.random.seed(42)
        X_train = np.random.randn(500, 5)  # 5 Features je Spektrum-Slice
        _model  = IsolationForest(contamination=0.08, random_state=42)
        _model.fit(X_train)
    return _model

def extract_features(power_db, n_slices=30):
    """Spektrum in Scheiben teilen, je Scheibe 5 statistische Features"""
...
def detect_anomalies(power_db, freqs_khz):
    ...
    # Slice-Grenzen für Plot berechnen
    n = len(freqs_khz)
    step = n // len(feats)
    anomaly_ranges = []
    ...
    return {
        ...
        "verdict":        "⚠️ Interferenzen erkannt" if n_anom > 0 else "✅ Spektrum unauffällig"
    }
...
def generate_demo_with_interference():
    ...
    # Schmalbandige Interferenz bei 420 kHz
  ...
  <body>
      ...
      <nav class="main-nav">
        ...
        <div class="nav-links">
          <a href="/" style="font-weight: bold"
            >SDR Spectrum Intelligence Dashboard</a
          >
    ...
    <script>
      ...
      function renderResults(d) {
        ...
        // Spektrum-Plot mit Anomalie-Highlights
        const traces = [
          {
        ...
        Plotly.newPlot(
          ...
          traces,
          {
            ...
            title: {
              text: "Spektrum mit Anomalie-Markierungen (rot)",
